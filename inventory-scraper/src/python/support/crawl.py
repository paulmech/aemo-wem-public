...
def crawl_folders(
    settings: Settings,
    fetcher: any = _fetch_directory_from_url,
    crawl_limit: int = DEFAULT_CRAWL_LIMIT,
):
    url = (
        settings.url
        if OPT_URL not in settings.options
        else settings.options[OPT_URL][0]
    )
    sleeptime = (
        DEFAULT_SLEEPTIME
        if OPT_SLEEPTIME not in settings.options
        else single_number(settings.options[OPT_SLEEPTIME], True)
    )
    ...
    while current_position < crawl_limit and current_position < len(all_dirs):
        ...
        if (
            OPT_MAX_DEPTH in settings.options
            and single_number(settings.options[OPT_MAX_DEPTH]) < current_depth
        ):
            current_position += 1
            continue
        ...
        for entry in dir_entries:
            if entry.is_directory:
                if (
                    OPT_NO_RECURSE in settings.options
                    or OPT_MAX_DEPTH in settings.options
                    and single_number(settings.options[OPT_MAX_DEPTH]) < current_depth
                ):
                ...
                if OPT_MAX_DAY_OLD in settings.options and entry.dt < (
                    datetime.now()
                    - timedelta(days=single_number(settings.options[OPT_MAX_DAY_OLD]))
                ):
                    continue
    ...
    if OPT_NO_EMPTY_DIRS in settings.options:
        discovered = list(
            filter(
                lambda item: item.is_directory
                and item.fileCount > 0
                or not item.is_directory,
                discovered,
            )
        )
