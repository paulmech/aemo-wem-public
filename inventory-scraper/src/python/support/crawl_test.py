...
def test_crawl_successful_descendency():
    ...
    assert 1 == len(
        list(
            filter(
                lambda link: link.is_directory
                and link.fileCount == 1
                and link.url == "https://fakeurl.com/public",
                results,
            )
        )
    )
    assert 1 == len(
        list(
            filter(
                lambda link: link.is_directory
                and link.fileCount == 2
                and link.url == "https://fakeurl.com/public/facilityScada/",
                results,
            )
        )
    )
    assert 1 == len(
        list(
            filter(
                lambda link: link.is_directory is False
                and link.fileSize == 2712
                and link.fileUrl == "https://fakeurl.com/public/somerandomfile.txt",
                results,
            )
        )
    )
    assert 1 == len(
        list(
            filter(
                lambda link: link.is_directory is False
                and link.fileSize == 83224
                and link.fileUrl
                == "https://fakeurl.com/public/facilityScada/FacilityScada_20230929.zip",
                results,
            )
        )
    )
    assert 1 == len(
        list(
            filter(
                lambda link: link.is_directory is False
                and link.fileSize == 91745
                and link.fileUrl
                == "https://fakeurl.com/public/facilityScada/FacilityScada_20230930.zip",
                results,
            )
        )
    )
...
def test_crawl_prohibit_descendency():
    fc = Fetcher([TEMPLATE_ENTRIES_ROOT, TEMPLATE_ENTRIES_DESCENDENT])
    settings = Settings(
        ["script.py", "https://fakeurl.com/public", "list-files", "--max-depth", "1"]
    )
    results = crawl_folders(settings, fc.fetcher)
...
def test_crawl_max_days_old():
    fc = Fetcher([TEMPLATE_ENTRIES_ROOT, TEMPLATE_ENTRIES_DESCENDENT])
    settings = Settings(
        ["script.py", "https://fakeurl.com/public", "list-files", "--max-days-old", "1"]
    )
    results = crawl_folders(settings, fc.fetcher)
    assert len(results) == 2
    assert 1 == len(
...
def test_crawl_no_recurse():
    fc = Fetcher([TEMPLATE_ENTRIES_ROOT, TEMPLATE_ENTRIES_DESCENDENT])
    settings = Settings(
        ["script.py", "https://fakeurl.com/public", "list-files", "--no-recurse"]
    )
    results = crawl_folders(settings, fc.fetcher)
    assert len(results) == 3
    assert 1 == len(
