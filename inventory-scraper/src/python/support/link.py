...
def _check_is_aemo_page(contents: str) -> any:
    if "wem.local" not in "".join( re.findall("<title>(.+?)</title>",contents)):
        raise Exception("Unable to find string 'wem.local' in URL title. Are you sure you have the right URL?")
...
def _create_summary_from_text_block(fstat: str) -> Summary:
    ...
    return Summary( \
        isodate = datetime.strptime(f"{summarylist[0]} {summarylist[1]} {summarylist[2]}",DATE_FORMAT), \
        is_directory = is_dir, \
