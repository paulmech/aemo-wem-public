...
def write_results_to_s3(results: list[Link], bucket: str, prefix: str, aws_request_id: str, dry_run: bool = False) -> str:
    # calculate the output filename
    if ( len(results) == 0 ):
        raise Exception("There are no results to write")
    elif bucket is None or len(bucket) == 0 or re.search("[^a-z0-9\\-\\.]",bucket) != None:
