...
def test_expected_output_path():
    ...
    output_path = write_results_to_s3([None], "bucket", "prefix///", "my_request_id", dry_run=True)
    assert output_path == f"s3://bucket/prefix/{datepart}/my_request_id/{datepart}.aemo-inventory.jsonl.gz"
...
def test_s3_upload(s3_client):
    ...
    upload_path = write_results_to_s3(results, "aemo-data-ap-southeast-2-111111111111", "testing", "request_id", dry_run=False)
    assert mock.upload_fileobj.called
    assert upload_path.startswith(f"s3://aemo-data-ap-southeast-2-111111111111/testing/{thenoo}/request_id/")
    assert upload_path.endswith("aemo-inventory.jsonl.gz")
    original_data = gzip.decompress( mock.upload_fileobj.call_args[0][0].read()).decode("UTF-8")
